...
export async function concurrency<T, R>(
  ...
  for (let index = 0; index < items.length; index++) {
    ...
    if (executing.length >= limit) {
      ...
      const executingIndex = executing.findIndex((p) => p === promise);
      if (executingIndex !== -1) {
        void executing.splice(executingIndex, 1);
      }
    }
  }
 ...
 */
export function promisify<TArguments extends unknown[], R>(
  function_: (...args: [...TArguments, (error: Error | null, result?: R) => void]) => void
